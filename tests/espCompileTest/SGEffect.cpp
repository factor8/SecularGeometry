...
void SGEffect::render() {
    ...
    if (_iter < _iterMax) {
        ...
        // if (DEBUG) { DEBUG_OUTPUT.print(F("Interval updated to "));DEBUG_OUTPUT.println(interval);  }
    }
...
void SGEffect::importFrame(Vector<uint32_t> f) {
  if (f.size() == frame.size()) {
    ...
    if (DEBUG) { DEBUG_OUTPUT.print(F("Frame Import Mismatch: "));DEBUG_OUTPUT.println(f.size());  }
  }  
}
...
void SGEffect::activate() {
    // if (DEBUG) { DEBUG_OUTPUT.print(F("Activating effect ID: "));DEBUG_OUTPUT.println(effect_id);  }
    _isActive = 1;
}
// Activate effect
void SGEffect::deactivate() {
    // if (DEBUG) { DEBUG_OUTPUT.print(F("Deactivating effect ID: "));DEBUG_OUTPUT.println(effect_id);  }
    _isActive = 0;
}
...
void SGEffect::status() {
  ...
  DEBUG_OUTPUT.println(F("<=== Effect Status Update ===>")); 
  DEBUG_OUTPUT.print(F("Iter: "));
  DEBUG_OUTPUT.println(_iter); 
  DEBUG_OUTPUT.print(F("IterMax: "));
  DEBUG_OUTPUT.println(_iterMax); 
  DEBUG_OUTPUT.print(F("Interval: "));
  DEBUG_OUTPUT.println(_interval); 
  DEBUG_OUTPUT.print(F("IntervalMax: "));
  DEBUG_OUTPUT.println(_intervalMax); 
  DEBUG_OUTPUT.print(F("Frequency: "));
  DEBUG_OUTPUT.println(_frequency);
  DEBUG_OUTPUT.print(F("firstRun: "));
  DEBUG_OUTPUT.println(_firstRun);
  DEBUG_OUTPUT.print(F("_lastRun: "));
  DEBUG_OUTPUT.println(_lastRun);
  DEBUG_OUTPUT.print(F("_willTransition: "));
  DEBUG_OUTPUT.println(_willTransition);
  

  DEBUG_OUTPUT.print(F("Primary Color: "));
  DEBUG_OUTPUT.println(_primary,HEX); 
  DEBUG_OUTPUT.print(F("Secondary Color: "));
  DEBUG_OUTPUT.println(_secondary,HEX); 
  DEBUG_OUTPUT.print(F("Tertiary Color: "));
  DEBUG_OUTPUT.println(_tertiary,HEX); 
  
  DEBUG_OUTPUT.println("-------");
  DEBUG_OUTPUT.print(F("panelsTotal: "));
  DEBUG_OUTPUT.println(_nodesTotal);  

  // DEBUG_OUTPUT.print(F("Phase: "));
